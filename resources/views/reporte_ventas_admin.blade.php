...
<div class="report-container">
    <div class="header">
        ...
        <p>Análisis Financiero y Operacional</p>
    </div>
    ...
    <div class="info-section">
        <div class="info-grid">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $total_ventas }} transacciones</span>
            </div>
        </div>
    </div>
    ...
    @if($resumen_por_tipo->isNotEmpty())
        <div class="summary">
            <h3>📊 Resumen de Ingresos por Tipo de Venta</h3>
            <table>
                <thead>
                <tr>
                    <th style="width: 40%;">Tipo de Venta</th>
                    <th style="width: 30%; text-align: center;">Cantidad de Ventas</th>
                    <th style="width: 30%; text-align: right;">Dinero Total Recaudado</th>
                </tr>
                </thead>
                <tbody>
                @php
                ...
                @foreach ($resumen_por_tipo as $tipo => $datos)
                    @php
                        $totalRecaudadoGeneral += $datos['dinero_total'];
                    @endphp
                    <tr>
                        <td style="font-weight: bold; color: #622D16;">{{ $tipo }}</td>
                        <td style="text-align: center;">{{ $datos['cantidad_ventas'] }}</td>
                        <td style="text-align: right; font-weight: bold;">${{ number_format($datos['dinero_total'], 0, ',', '.') }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
            ...
            <div class="stats-highlight">
                <strong>Total general recaudado en el período:</strong> ${{ number_format($totalRecaudadoGeneral, 0, ',', '.') }}
            </div>
        </div>
    @endif
    ...
    <table>
        <thead>
        <tr>
            <th style="width: 18%;">ID Venta</th>
            <th style="width: 25%;">Fecha/Hora</th>
            <th style="width: 20%;">Atendido por</th>
            <th style="width: 17%; text-align: center;">Tipo de Venta</th>
        ...
        @forelse ($ventas as $venta)
            <tr>
                ...
                <td>{{ $venta->fecha_hora_venta->format('d/m/Y H:i') }}</td>
                <td>{{ $venta->usuario->nombre ?? 'N/A' }}</td>
                <td style="text-align: center;">{{ $venta->tipoVenta->tipo ?? 'N/A' }}</td>
                <td style="text-align: right; font-weight: bold; color: #622D16;">${{ number_format($venta->total, 0, ',', '.') }}</td>
            </tr>
        @empty
            <tr class="empty-row">
                <td colspan="5">
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 15%;">Tipo/Tamaño</th>
        ...
        <tbody>
        @forelse ($ventas_producto as $vp)
            <tr>
                <td>{{ $vp->producto->nombre ?? 'N/A' }}</td>
                <td>
                    {{ $vp->producto->tipoProducto->tipo ?? '' }} 
                    ({{ $vp->producto->tamanoProducto->tamano ?? '' }})
                </td>
                <td>{{ $vp->venta_id_venta }}</td>
                <td>{{ $vp->venta->fecha_hora_venta->format('d/m/Y H:i') }}</td>
                <td style="text-align: right;">{{ $vp->cantidad_vendida }}</td>
                <td style="text-align: right; font-weight: bold;">${{ number_format($vp->subtotal, 0, ',', '.') }}</td>
            </tr>
        @empty
            <tr class="empty-row">
                <td colspan="6">
                    No se encontraron detalles de productos vendidos en el período.
                </td>
            </tr>
        @endforelse
        </tbody>
